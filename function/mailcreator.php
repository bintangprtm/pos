...
include "db.php";
$id=$formcoe;
$data=mysqli_fetch_array(mysqli_query($db,"SELECT * FROM data_pengajuan where form_code='$id'"));
...
if($data['progres']=='1'){	$sapaan="Dear $atasan[nama]";}elseif($data['progres']=='2'){ $sapaan="Dear Ibu Wismayanti";}elseif($data['progres']=='3'){ $sapaan="Dear Bapak Pratama";}elseif($data['progres']=='4'){$sapaan="Dear $pembelian[nama]";}elseif($data['progres']=='5'){$sapaan="Dear $pengaju[nama]";};
	...
	$laporan .="</tr>";
	$laporan .="<tr>";
	$laporan .="<td colspan='3'><b>Terdapat pengajuan baru</b> dengan detail sebagai berikut :</td>";
	$laporan .="</tr>";		
	$laporan .="<tr>";
...
try {
	...
	if($data['progres']=='1'){	$to = "$atasan[email]";}elseif($data['progres']=='2'){ $to="bayu.nugraha@ofon.co.id";}elseif($data['progres']=='3'){ $to = "satriyo.wibowo@ofon.co.id";}elseif($data['progres']=='4'){$to= "$pembelian[email]";}elseif($data['progres']=='5'){$to="$pengaju[email]";};
	$mail->AddAddress($to);
